...
select COUNT(PersonelId) from TBL_Personel where PersonelAd like '[D-P]%' and YEAR(PersonelDoðumTarihi)<=1993
...
-- Soyadý H ile baþlayan kaç oyuncu vardýr kolon adý oyuncu sayýsý olsun?
select COUNT(FilmOyuncuId) as 'Oyuncu Sayýsý' from TBL_FilmOyunculari where FilmOyuncuSoyadi like '[H]%'
...
--DiðerÖdevSorgusu 'CATU' 'Duman' 'Peric' ýd li müþteriler tarafýnfan istenmiþ 2 nolu kargo firmasý ile 
--taþýnmamýþ kargo ücreti 20 ile 200 dolar arasý olan sipariþlere toplam ne kadarlýk 
--kargo ödemesi yapýlmýþtýr
